...
class StoringPostCodeEncryptedIntegrationTest extends KafkaEncryptedTestBase {
  ...
  @Autowired
  private KafkaTemplate<String, IndividualEventV1> individualEventV1KafkaTemplate;

  private Consumer<String, IndividualEventV1> individualEventV1Consumer;

  @Value("${spring.kafka.consumer.individual-party-v2-encrypted-topic}")
  ...
  public void initTest() {
    ...
    individualEventV1Consumer = new DefaultKafkaConsumerFactory(consumerProps, new StringDeserializer(),
        specificRecordSecuredKafkaAvroDeserializer).createConsumer(CONSUMER_GROUP_ID, CLIENT_PREFIX);

    individualEventV1Consumer.subscribe(new ArrayList<>(List.of(topic)));

    ConcurrentMessageListenerContainer<?, ?> container = (ConcurrentMessageListenerContainer<?, ?>) kafkaListenerEndpointRegistry
        .getListenerContainer("IndividualInfoListenerEncrypted");
  ...
  private void processKafkaMessage(IndividualEventV1 individualEventV1) {
    individualEventV1KafkaTemplate.send(new ProducerRecord<>(topic, 1,null, individualEventV1));
